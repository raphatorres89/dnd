...
public class WarlockClassesGenerator extends AbstractClassesGenerator {
    ...
    @Override
    protected void generateSpells(Set<SpellEnum> spells, Integer npcLevel, NpcStats npcStats) { // Assumindo Int Mod como parâmetro

        // --- VARIÁVEIS DE SEGURANÇA PARA EVITAR TRAVAMENTO ---
        final int MAX_ATTEMPTS_PER_SPELL = 20;
        int attempts;
        ...
        int addedCantrips = 0;
        attempts = 0;
        // Tenta adicionar 'cantrips' magias únicas, com limite de tentativas
        while (addedCantrips < cantrips && attempts < cantrips * MAX_ATTEMPTS_PER_SPELL) {
            SpellEnum newSpell = addUniqueSpell(spells, 0);
            if (newSpell != null) {
                spells.add(newSpell);
                addedCantrips++;
            }
        ...
        // Nível de Mago + Modificador de Inteligência (mínimo de 1)
        ...
        // Mago é conjurador completo: (npcLevel + 1) / 2 (divisão inteira)
        int maxSpellLevel = Math.min(9, (npcLevel + 1) / 2);

        // --- 4. DISTRIBUIÇÃO PONDERADA DE MAGIAS PREPARADAS ---
        int addedPreparedSpells = 0;
        attempts = 0;

        // Tenta adicionar 'maxPreparedSpells' magias únicas, com limite de tentativas
        while (addedPreparedSpells < maxPreparedSpells && attempts < maxPreparedSpells * MAX_ATTEMPTS_PER_SPELL) {
            ...
            if (maxSpellLevel == 1) {
                ...
                // Lógica de Distribuição Ponderada (prioriza níveis mais baixos)
                int potentialLevel = (int) (Math.random() * maxSpellLevel) + 1;

                // 75% das magias de círculos <= maxSpellLevel - 1
                if (addedPreparedSpells < maxPreparedSpells * 0.75) {
            ...
            SpellEnum newSpell = addUniqueSpell(spells, selectedLevel);
            if (newSpell != null) {
                spells.add(newSpell);
                addedPreparedSpells++;
            }
